...
export const createPlan = async (obj) => {
  if (obj.date) {
    ...
    if (inputDate.getTime() <= getTodayIndo().getTime()) {
      throw new Error(
        "Hanya boleh membuat plan untuk besok atau tanggal setelahnya (H+1).",
      );
    }
  }
  ...
  if (existing.length > 0) {
    throw new Error(
      "Plan untuk tanggal tersebut sudah dibuat. Mohon untuk memilih tanggal lain! :D",
    );
  }
...
export const getPlanById = async (id) => {
  const { data, error } = await supabase
    ...
    .eq("id", id)
    .order("time", { foreignTable: "plan_item", ascending: true })
    .single();
...
export const updatePlan = async (id, updateData) => {
  ...
  if (currentPlanDate.getTime() <= getTodayIndo().getTime()) {
    throw new Error(
      "Plan hari ini atau yang sudah lewat tidak dapat diubah kembali.",
    );
  }
  ...
  if (updateData.date) {
    ...
    if (newInputDate.getTime() <= getTodayIndo().getTime()) {
      throw new Error(
        "Hanya boleh mengubah plan ke tanggal besok atau setelahnya (H+1).",
      );
    }
  }
...
export const getPlansByMonth = async ({
  showAll = false,
  areaGroup = false,
  id_user,
  area,
  ...
  if (areaGroup && !showAll) {
    selectString += `, branch:id_branch!inner(
      id,
      name,
      area:id_area!inner(
        id,
  ...
  if (areaGroup && !showAll) {
    if (area) {
      query = query.eq("id_branch.id_area.area", area);
    } else {
  ...
  if (filterUser) {
    query = query.eq("id_user", filterUser);
  }
